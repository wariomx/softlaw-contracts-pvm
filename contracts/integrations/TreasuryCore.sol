...
contract TreasuryCore is AccessControl, ReentrancyGuard, Pausable {
    ...
    bytes32 public constant MARKETPLACE_CONTRACT = keccak256("MARKETPLACE_CONTRACT");
    ...
    event SystemAddressUpdated(string indexed component, address indexed oldAddress, address indexed newAddress);
    ...
    event PayoutScheduled(address indexed recipient, uint256 amount, string reason);
    ...
    function processRegistrationPayment(
        ...
        require(slawToken.balanceOf(user) >= feeAmount, "Insufficient SLAW balance");
        
        // Transfer fee to treasury
        bool success = slawToken.treasuryTransferFrom(user, address(this), feeAmount);
    ...
    function processLicensePayment(
        ...
        require(feeConfig.isActive, "Fees not active");
        require(licensor != address(0) && licensee != address(0), "Invalid addresses");
        
        uint256 totalFee = feeConfig.licensingBaseFee + customAmount;
        require(slawToken.balanceOf(licensee) >= totalFee, "Insufficient SLAW balance");
        ...
        bool success = slawToken.treasuryTransferFrom(licensee, address(this), totalFee);
        ...
        emit LicensePayment(licensor, licensee, licenseId, totalFee, licensorShare, protocolShare);
    ...
    function processMarketplaceFee(
        ...
        uint256 saleAmount,
        uint256 itemId
    ) external onlyRole(MARKETPLACE_CONTRACT) nonReentrant whenNotPaused returns (uint256 netAmount) {
        require(feeConfig.isActive, "Fees not active");
        require(seller != address(0) && buyer != address(0), "Invalid addresses");
        require(saleAmount > 0, "Invalid sale amount");
        
        // Calculate marketplace fee
        uint256 marketplaceFee = (saleAmount * feeConfig.marketplaceFeeRate) / 10000;
        netAmount = saleAmount - marketplaceFee;
        
        require(slawToken.balanceOf(buyer) >= saleAmount, "Insufficient buyer balance");
        
        // Transfer total amount from buyer to treasury
        bool success = slawToken.treasuryTransferFrom(buyer, address(this), saleAmount);
     ...
     */
    function _schedulePayout(address recipient, uint256 amount, string memory reason) internal {
    ...
    function claimPayout() external nonReentrant whenNotPaused {
        ...
        require(slawToken.balanceOf(address(this)) >= amount, "Insufficient treasury balance");
    ...
    function updateFeeConfig(
        ...
        emit FeeConfigUpdated(_registrationFee, _licensingBaseFee, _marketplaceFeeRate, _liquidityFeeRate);
    }
    ...
    function emergencyWithdraw(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(slawToken.balanceOf(address(this)) >= amount, "Insufficient balance");
