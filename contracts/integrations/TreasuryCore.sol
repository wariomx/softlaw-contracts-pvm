...
contract OptimizedTreasuryCore is AccessControl, ReentrancyGuard, Pausable {
    ...
    event EscrowDeposited(uint256 indexed disputeId, address indexed depositor, uint256 amount);
    event EscrowReleased(uint256 indexed disputeId, address indexed recipient, uint256 amount);
    event ArbitratorFeePaid(address indexed arbitrator, uint256 indexed disputeId, uint256 amount);
    event AwardDistributed(address indexed winner, uint256 indexed disputeId, uint256 amount);
    event SecurePaymentProcessed(address indexed recipient, uint256 amount, bool success);
    event OperatorAuthorized(address indexed operator, bool authorized);

    constructor(address _admin, address _slawToken) {
     ...
     */
    function deposit(uint256 amount, string calldata purpose) external nonReentrant whenNotPaused {
        require(amount > 0, "Amount must be > 0");
        
        // Transfer SLAW to treasury
        _safeTransferFrom(address(slawToken), msg.sender, address(this), amount);
     ...
     */
    function withdraw(uint256 amount, string calldata purpose) external nonReentrant whenNotPaused {
        require(amount > 0, "Amount must be > 0");
    ...
    function payLicenseFee(
        ...
        require(amount > 0, "Amount must be > 0");
        require(authorizedOperators[msg.sender] || hasRole(OPERATOR_ROLE, msg.sender), "Not authorized");
    ...
    function distributeRevenue(
        ...
        require(totalAmount <= totalFeesCollected, "Insufficient fees collected");
    ...
    function getDisputeEscrow(uint256 disputeId) external view returns (uint256) {
    ...
    function authorizeOperator(address operator, bool authorized) external onlyRole(TREASURY_ADMIN) {
