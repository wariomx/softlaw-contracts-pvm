...
contract PersonalizedWrappedIPToken is ERC20 {
    ...
    constructor(
        ...
        string(abi.encodePacked(_getPrefix(_creatorName), _getPrefix(_ipTitle)))
    ) {
        nftId = _nftId;
    ...
    function _getPrefix(string memory _name) internal pure returns (string memory) {
...
contract WrappedIPManager is AccessControl, ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    function _deployWrappedToken(
        ...
        return address(new PersonalizedWrappedIPToken(
            nftId,
            nftContract,
            creator,
            address(this),
    ...
    function unwrapIP(bytes32 ipId) external nonReentrant whenNotPaused {
        ...
        _updateUnwrappingMetrics(msg.sender, ipInfo.totalSupply, ipInfo.initialPrice);
    ...
    function setDisputeResolver(address _disputeResolver) external onlyRole(DEFAULT_ADMIN_ROLE) {
