...
contract OptimizedLiquidityManager is AccessControl, ReentrancyGuard, Pausable {
    ...
    event LiquidityRemoved(bytes32 indexed poolId, address indexed user, uint256 liquidity);
    event TokensSwapped(bytes32 indexed poolId, address indexed user, uint256 amountIn, uint256 amountOut);
    event FeesCollected(bytes32 indexed poolId, address indexed user, uint256 feeAmount);
    ...
    function removeLiquidity(
        ...
        UserPosition storage position = userPositions[poolId][msg.sender];
        require(position.liquidityTokens >= liquidity, "Insufficient liquidity");
    ...
    function _getAmountOut(
        ...
        uint256 amountInWithFee = amountIn * (10000 - TRADING_FEE - PROTOCOL_FEE);
    ...
    function getUserPosition(bytes32 poolId, address user) external view returns (UserPosition memory) {
    ...
    function collectFees(bytes32 poolId) external nonReentrant returns (uint256 feeAmount) {
        UserPosition storage position = userPositions[poolId][msg.sender];
        require(position.liquidityTokens > 0, "No liquidity position");
        
        // Calculate fees earned (simplified calculation)
        LiquidityPool storage pool = pools[poolId];
        uint256 userShare = (position.liquidityTokens * 10000) / pool.totalLiquidity;
