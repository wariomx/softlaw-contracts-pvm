...
contract SLAWToken is ERC20, AccessControl, Pausable, ReentrancyGuard {
    ...
    function _claimRewards(address user) internal {
        ...
        uint256 rewardAmount = (userStake.amount *
            stakingRewardRate *
            stakingDuration) / (BASIS_POINTS * SECONDS_PER_YEAR);
    ...
    function calculatePendingRewards(
        ...
        uint256 stakingDuration = block.timestamp - userStake.lastRewardClaim;
        return
            (userStake.amount * stakingRewardRate * stakingDuration) /
    ...
    function distributeTreasuryFunds(
        ...
        require(
            totalAmount <= treasuryReserve,
            "Insufficient treasury reserve"
        );
        require(
            balanceOf(treasuryWallet) >= totalAmount,
