...
contract WrappedIPToken is ERC20, Ownable {
    ...
    function mint(address to, uint256 amount) external onlyOwner {
        require(
            ipDetails.circulatingSupply + amount <= ipDetails.totalSupply,
            "Exceeds max supply"
        );
        _mint(to, amount);
    ...
    function completeRedemption() external {
        ...
        require(
            block.timestamp >=
                redemptionTimestamps[msg.sender] + REDEMPTION_DELAY,
        ...
        if (ipDetails.circulatingSupply == 0) {
            IERC721(ipDetails.nftContract).transferFrom(
                treasury,
                msg.sender,
                ipDetails.nftId
            );
