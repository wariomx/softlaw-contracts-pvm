...
contract SoftlawDisputeResolution is AccessControl, ReentrancyGuard, Pausable {
    ...
    function fileDispute(
        ...
        emit DisputeFiled(disputeId, msg.sender, defendant, disputeType, claimedDamages);
    ...
    function depositEscrow(uint256 disputeId, uint256 amount) external nonReentrant {
        ...
        treasury.transferFrom(msg.sender, address(this), amount);
    ...
    function startMediation(
        ...
        require(dispute.status == DisputeStatus.FILED, "Invalid status for mediation");
        ...
        treasury.payLicenseFee(address(0), dispute.plaintiff, disputeId, feePerParty);
        treasury.payLicenseFee(address(0), dispute.defendant, disputeId, feePerParty);

        dispute.status = DisputeStatus.MEDIATION;
        dispute.assignedMediator = mediator;

        emit DisputeStatusChanged(disputeId, DisputeStatus.FILED, DisputeStatus.MEDIATION);
    ...
    function startArbitration(
        ...
        treasury.distributeIncentives(
            _asSingletonArray(arbitrator),
            _asSingletonArray(arbitrators[arbitrator].feePerCase)
        );

        dispute.status = DisputeStatus.ARBITRATION;
        dispute.assignedArbitrator = arbitrator;

        emit DisputeStatusChanged(disputeId, dispute.status, DisputeStatus.ARBITRATION);
    ...
    function submitArbitrationDecision(
        ...
        emit DisputeStatusChanged(disputeId, DisputeStatus.ARBITRATION, DisputeStatus.RESOLVED);
    }
    ...
    function executeResolution(uint256 disputeId) external nonReentrant {
        ...
        require(dispute.enforceable, "Not enforceable");
        require(dispute.escrowAmount >= dispute.awardAmount, "Insufficient escrow");

        // Transfer award from escrow to winner
        if (dispute.awardAmount > 0) {
            treasury.transfer(dispute.winner, dispute.awardAmount);
        }
        ...
        if (remaining > 0) {
            ...
            treasury.transfer(dispute.plaintiff, halfRemaining);
            treasury.transfer(dispute.defendant, remaining - halfRemaining);
        }

        dispute.status = DisputeStatus.ENFORCED;
        emit DisputeStatusChanged(disputeId, DisputeStatus.RESOLVED, DisputeStatus.ENFORCED);
    }
    ...
    function getIPDisputes(uint256 ipId) external view returns (uint256[] memory) {
