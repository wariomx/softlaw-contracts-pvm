...
contract CopyrightLicensing is CopyrightsRegistry, ERC2981 {
    ...
    event AutoTokenizationTriggered(
    ...
    function tokenizeCopyright(
        ...
        require(
            copyrights[copyrightId].economicRightsOwner == msg.sender,
            "Not owner"
        );
    ...
    function getCopyrightInfo(
        ...
        external
        view
        validTokenId(copyrightId)
        returns (
            CopyrightWrapper memory copyright,
            bool tokenized,
            address wrappedToken,
            uint256 rewards,
     ...
     */
    function getEnhancedLicense(
        uint256 licenseId
    ) external view returns (EnhancedLicense memory) {
    ...
    /**
     * @dev Validate that granted rights are owned by the licensor
     * @param ipId Copyright ID
     * @param grantedRights Rights being licensed
     */
    function _validateGrantedRights(
        ...
        require(grantedRights.owner == msg.sender, "Invalid rights owner");
    }

    /**
     * @dev Trigger automatic tokenization for popular copyrights
     */
    function _triggerAutoTokenization(
        uint256 copyrightId,
        address owner
    ) internal {
        ...
        try
            treasury.wrapCopyrightNFT(
                address(this),
                copyrightId,
                defaultSupply,
                defaultPrice,
                string(
                    abi.encodePacked("Auto-wrapped Copyright #", copyrightId)
                )
            )
     ...
     */
    function emergencyDisableAutoTokenization(
        uint256 copyrightId
    ) external onlyOwner {
