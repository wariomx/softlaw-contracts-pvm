...
interface IWrappedIPToken is IERC20 {
    // Events
    event IPWrapped(uint256 indexed nftId, address indexed nftContract, uint256 totalSupply);
...
interface IWrappedIPFactory {
    ...
    function isValidWrappedToken(address tokenAddress) external view returns (bool);
}
...
interface ISoftlawTreasury {
    // Events
    event IPWrapped(uint256 indexed nftId, address indexed nftContract, address indexed tokenAddress);
    event LiquidityPoolCreated(address indexed pairAddress, address indexed ipToken);
    event RegistrationPaid(address indexed user, uint256 indexed nftId, uint256 amount);
    event LicensePaid(address indexed licensor, address indexed licensee, uint256 amount);
    ...
    function payLicenseFee(address licensor, address licensee, uint256 licenseId, uint256 amount) external;
    ...
    function getSystemMetrics() external view returns (uint256, uint256, uint256, uint256, uint256);
}
